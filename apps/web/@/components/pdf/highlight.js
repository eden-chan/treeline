...
var getRectFromOffsets = function (textDiv, startOffset, endOffset) {
	try {
		...
		// Check if endOffset is within the valid range
		if (endOffset <= firstChild.textContent.length) {
...
var getTextFromOffsets = function (
	nodes,
	pageIndex,
	startDivIdx,
	startOffset,
...
var Tracker = function (_a) {
	...
	var onMouseUpHandler = function () {
		...
		var startDivSiblings = [].slice.call(
		...
		switch (rangeType) {
			...
			case SelectionRange.DifferentPages:
				const offsets = getRectFromOffsets(
					startDiv,
					startOffset,
					startDiv.textContent.length,
				);
...
var highlightPlugin = function (props) {
	...
	var handleMouseDown = function (textLayerRender) {
		return function (e) {
			...
			var selectEnd = textLayer.querySelector(
	...
	var handleMouseUp = function (textLayerRender) {
		return function (e) {
			...
			var selectEnd = textLayerRender.ele.querySelector(
				".".concat(TEXT_LAYER_END_SELECTOR),
			);
			if (selectEnd) {
	...
	var onTextLayerRender = function (e) {
		...
		if (e.status === core.LayerRenderStatus.PreRender) {
			...
			var selectEndEle = textEle.querySelector(
				".".concat(TEXT_LAYER_END_SELECTOR),
			);
			if (selectEndEle) {
