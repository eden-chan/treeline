import type { LTWHP } from "../../../../../@/components/pdf/types.js";
...
const getBoundingRect = (clientRects: Array<LTWHP>): LTWHP => {
	...
	const optimal = rectsWithSizeOnFirstPage.reduce((res, rect) => {
		return {
			X0: Math.min(res?.X0 ?? 0, rect?.X0 ?? 0),
			X1: Math.max(res?.X1 ?? 0, rect.X1 ?? 0),

			Y0: Math.min(res?.Y0 ?? 0, rect?.Y0 ?? 0),
			Y1: Math.max(res?.Y1 ?? 0, rect.Y1 ?? 0),
