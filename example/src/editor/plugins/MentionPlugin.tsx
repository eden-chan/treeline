...
function useMentionLookupService(
  ...
  candidateTexts: Array<LookupServiceResult>,
): Array<LookupServiceResult> {
  ...
  useEffect(() => {
    ...
    const newResults = candidateTexts
      .filter(
        (mention: LookupServiceResult) =>
...
export default function MentionPlugin({
  ...
  const {
    documents,
    highlightsWithComments: highlights,
    users,
  } = useBundleContext();
  ...
  const candidateHighlightedText: Array<LookupServiceResult> = useMemo(() => {
    return (
      highlights
        ...
        .map((highlight) => ({
          id: highlight.id,
          name: highlight.content.text,
        })) ?? []
    );
  }, [highlights]);

  const candidateComments: Array<LookupServiceResult> = useMemo(() => {
    return highlights.flatMap(
      (highlight): Array<LookupServiceResult> =>
        highlight.comments?.map((comment: Comment) => ({
          id: comment.id,
          name: comment.text,
        })) || [],
    );
  ...
  const results: Array<LookupServiceResult> = useMentionLookupService(
    queryString,
    candidateTexts,
  );
  ...
  const options: Array<MentionTypeaheadOption> = useMemo(
    () =>
      results
        .map((result) => new MentionTypeaheadOption(result.name, result.id))
        .slice(0, SUGGESTION_LIST_LENGTH_LIMIT),
    [results],
  );
      ...
      menuRenderFn={(
        ...
        anchorElementRef.current && results.length
          ? ReactDOM.createPortal(
              <div
                className={`${styles.typeaheadPopover} ${styles.mentionsMenu}`}
              >
                <ul>
